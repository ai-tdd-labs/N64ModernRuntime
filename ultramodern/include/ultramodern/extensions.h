...
namespace extensions {
    ...
    // Optional helper for recomp projects that need to avoid CPU/GPU race conditions.
    // Waits until the renderer thread reports that it parsed the given displaylist.
    bool wait_for_displaylist_parsed(PTR(u64) displaylist, uint32_t timeout_ms);
}
}
#endif
